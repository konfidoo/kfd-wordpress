...
registerBlockType( 'cgb/block-konfidoo', {
	...
	edit: ( {attributes, setAttributes} ) => {
		...
		// Helper function to get the effective Project ID (block-specific or global fallback)
		const getEffectiveProjectId = () => {
			return attributes.projectId || (cgbGlobal && cgbGlobal.globalProjectId) || '';
		};
		...
		return (
			<div>
					...
					>
						<PanelRow>
							<TextControl
								...
								placeholder={ cgbGlobal && cgbGlobal.globalProjectId ? `Global: ${cgbGlobal.globalProjectId}` : 'Enter Project ID' }
				...
				<div className="wp-block-kfd-placeholder">
					...
					{ getEffectiveProjectId() && attributes.configurationId && (
						<div>
							<strong>Project: </strong>
							{getEffectiveProjectId()}
							{!attributes.projectId && cgbGlobal && cgbGlobal.globalProjectId && (
								<span style={{color: '#666', fontSize: '0.9em'}}> (Global)</span>
							)}
							<br/>
							<strong>Konfiguration: </strong>
							{attributes.configurationId}
							<br/>
							<a href={'https://dev.konfidoo.de/cms/mandant/' + getEffectiveProjectId()} target="_blank">
								Projekt öffnen</a>
					...
					{ !getEffectiveProjectId() && (
						<div>Bitte eine Projekt ID angeben{cgbGlobal && cgbGlobal.globalProjectId ? ' oder in den Plugin-Einstellungen konfigurieren' : ''}</div>
					) }
					{ !attributes.configurationId && (
	...
	save: ( props ) => {
		// Helper function to get the effective Project ID (block-specific or global fallback)
		const getEffectiveProjectId = () => {
			return props.attributes.projectId || (cgbGlobal && cgbGlobal.globalProjectId) || '';
		};
